...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non MONET unit input in lnReqAmount, we generate the URI with MONET as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(MonetaUnits::format(MonetaUnits::MONET, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
